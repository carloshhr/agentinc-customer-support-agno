...
function Detail({ detail, csrfToken, onBack, onSent, onExpired }: { detail: ThreadDetail; csrfToken: string; onBack: () => void; onSent: (detail: ThreadDetail | null, pending: boolean) => void; onExpired: () => void }) {
  ...
  return <section className="detail" aria-label="Thread detail">
    ...
    <Compose thread={detail} csrfToken={csrfToken} onSent={onSent} onExpired={onExpired} />
  </section>
}
...
export default function App() {
  ...
  useEffect(() => {
    supportApi.getSession().then(({ operator: restored, csrf_token }) => {
      ...
      return supportApi.listThreads().then(({ threads: loaded }) => setThreads(loaded)).catch((reason: unknown) => {
        if (isUnauthorized(reason)) expired()
        else setError(errorMessage(reason, 'The inbox could not be loaded.'))
      })
  ...
  function expired() { setOperator(null); setCsrfToken(''); setThreads([]); setDetail(null); setError('Your session expired. Please sign in again.') }
  ...
  async function selectThread(sessionId: string) {
    try {
      ...
      setPending(false)
    } catch (reason) { if (isUnauthorized(reason)) expired(); else setError(errorMessage(reason, 'This thread could not be loaded.')) }
  }

  async function sent(result: ThreadDetail | null, isPending: boolean) {
    setPending(isPending)
    if (result) setDetail(result)
    try { setThreads((await supportApi.listThreads()).threads) } catch (reason) { if (isUnauthorized(reason)) expired(); else setError(errorMessage(reason, 'The inbox was sent, but could not be refreshed.')) }
  }
  ...
  return <main className={detail ? 'inbox selected' : 'inbox'}>
    <aside className="thread-list" aria-label="Support threads">
      <header><h1>Support Inbox</h1><p>Signed in as {operator.display_name}</p><button onClick={logout}>Sign out</button></header>
    ...
    <section className="panel">
      ...
      {detail ? <Detail detail={detail} csrfToken={csrfToken} onBack={() => setDetail(null)} onSent={sent} onExpired={expired} /> : <div className="empty"><h2>Select a thread</h2><p>Choose a conversation to inspect its safely normalized emails.</p></div>}
    </section>
  </main>
}
