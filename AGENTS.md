# Customer Support AgentOS — Railway template
...
**Customer Support AgentOS: the AgentOS platform with a public support showcase.** Built on the [Agno framework](https://docs.agno.com), it exposes a production API through **REST**, human chat interfaces (Slack is wired in; WhatsApp/Telegram/Discord mirror the pattern), and **MCP** at `/mcp` for AI apps (claude.ai, ChatGPT, Cursor, Claude Code). **Customer Support** is the public showcase team: it coordinates private Order Support, Product Support, and Returns & Refunds Support specialists. Support Inbox and its BFF form the bounded external operator surface for that team; Support Insights is a separate operator-facing reporting agent. Its access follows deployment-wide AgentOS agent-run authorization, not a component-specific administrator role. Agno and Platform Builder, Platform Manager, and Platform Engineer remain the platform control plane: Agno fronts those three agents, which build at runtime, watch the runtime, and read the source. The platform grows through two lanes. **Lane 1: coding agents change the source** — eight coding-agent skills cover platform setup, the full agent development lifecycle, and production deploy, governed by git. **Lane 2: the platform builds at runtime** — Platform Builder composes agents, teams, and workflows from a safe registry of reviewed blocks, governed by the Studio catalog (drafts, publish, versions, archive/restore). [`app/registry.py`](app/registry.py) is the membrane between the lanes: every capability a built component can carry is declared there by a reviewed code change, and the builder composes but never expands it. Postgres (pgvector) handles persistence for sessions, memory, and knowledge. Runs locally via Docker; this template deploys to Railway with a single script and is the reference sibling of the `agentos-*` deployment family — see [Portable core vs. deploy layer](#portable-core-vs-deploy-layer).

## Architecture

```
Customer Support AgentOS  (app/main.py)
├── Customer Support  (teams/customer_support.py) — public showcase team for validated customer email
│   ├── Order Support                         — private order, fulfillment, and tracking specialist
│   ├── Product Support                       — private catalog and sizing specialist
│   └── Returns & Refunds Support             — private deterministic and approval-gated refund specialist
├── Support Inbox     (app/support_inbox.py)  — bounded AgentOS browser API and hosted frontend route
│   └── Support Inbox BFF (support-inbox/bff/) — separate operator authentication, audit, and proxy boundary
├── Support Insights  (agents/support_insights.py) — separate operator-facing, on-demand aggregate reporting agent; access follows deployment-wide AgentOS agent-run authorization
├── Agno              (teams/lead.py)        — control-plane front door: LearningMachine + notes + web tools + ask_user + studio_runners;
│                                               members = the three platform agents below; runs Studio-built components on demand
├── Platform Builder  (agents/builder.py)     — control plane: lane 2 StudioTools over the safe registry; builds end published
├── Platform Manager  (agents/manager.py)     — control plane: read-only runtime lens + deployment-check tools
├── Platform Engineer (agents/engineer.py)    — control plane: read-only source lens
...
- JWT auth on whenever `RUNTIME_ENV` is anything but `dev` (so production deploys, which default to `prd`, are gated by default). Support Insights has no component-specific administrator authorization guard; its operator-facing access follows this deployment-wide AgentOS agent-run authorization boundary.

## Key Files

| File | Purpose |
|------|---------|
| [`app/main.py`](app/main.py) | AgentOS entrypoint — registers the Customer Support and Agno teams; Support Insights and the three platform agents; support demo seeding; Support Inbox routes; conditional Slack; conditional MCP OAuth; and the JWT gate. |
...
| [`app/offload.py`](app/offload.py) | `result_store` — one `ResultStore`, 7-day TTL, carried by Agno and the three platform agents. Big tool results become searchable stored files instead of context. |
...
| [`teams/customer_support.py`](teams/customer_support.py) | Customer Support — the public typed showcase team; coordinates the three private specialists and persists validated support interactions. |
| [`agents/order_support.py`](agents/order_support.py) | Order Support — private specialist for customer-owned order, fulfillment, and tracking facts. |
| [`agents/product_support.py`](agents/product_support.py) | Product Support — private specialist for catalog, material, sizing, care, and supported policy retrieval. |
| [`agents/refund_support.py`](agents/refund_support.py) | Returns & Refunds Support — private specialist for deterministic automatic and administrator-approved refund routes. |
| [`agents/support_insights.py`](agents/support_insights.py) | Support Insights — separate operator-facing registered agent for on-demand aggregate support reporting; access follows deployment-wide AgentOS agent-run authorization rather than a component-specific administrator role. |
| [`app/support_demo.py`](app/support_demo.py) | Deterministic model-free Customer Support demo threads seeded at startup for local inspection. |
| [`app/support_inbox.py`](app/support_inbox.py) | Bounded Customer Support browser API and hosted Support Inbox frontend route; allow-lists safe thread data and requires the designated BFF scopes outside dev. |
| [`frontend/support-inbox/`](frontend/support-inbox/) | Independent React/Vite Support Inbox operator frontend; its built assets are served under `/support-inbox/`. |
| [`support-inbox/bff/`](support-inbox/bff/) | Separate Support Inbox BFF: operator session authentication, CSRF/origin controls, audit persistence, and bounded proxy calls to AgentOS. |
...
- `demo` — the model-backed portfolio gate for Customer Support and operator-facing Support Insights.
...
Run with `python -m evals --tag smoke`, `python -m evals --tag demo`, `python -m evals --tag release`, or `python -m evals --name <case>`. The `demo` gate is distinct from the deterministic, model-free seeded Support Inbox walkthrough, which displays fixed local threads without running eval cases. Add `--json-output out.json` when a workflow or coding agent needs machine-readable results. Results log to Postgres via `db=eval_db` so history is visible at os.agno.com.
...
**The world is shared, the self is private.** Notes (`FileSystem` namespace `shared-notes` — files land in Postgres under the `fs` schema) and entities (namespace `global`) are shared by everyone on the platform; user profile and user memory are per-user (agentic mode, so their tools only exist when a run carries a user id). The self also spans the control-plane agents: all three Agno members wire the same per-user profile/memory stores — the one `shared_learning` machine in [`app/learning.py`](app/learning.py) — so what Agno learns about a user follows them to every control-plane agent. It spans lane 2 as well: that machine is registered under the name `shared-learning`, so a component Platform Builder builds can be wired to the same self and pick the thread up from there. Corrections supersede rather than accumulate — stating a new fact retires the contradicted one (a judged model call in the write path), and facts render with as-of dates.

**Identity decides what stays private.** A run's identity always wins: Slack runs as the sender, production runs as the JWT `sub`, PATs as `sa:<name>`. `user_id="anonymous-user"` (on the Agno team and the three platform agents) is only the fallback for anonymous local runs (dev `/mcp`, evals) — without it they would silently lose the profile/memory tools. One caveat to know: the built-in MCP OAuth identifies the *connector registration*, not the person — claude.ai and ChatGPT connect as different `__oauth__:<client_id>` principals, so the same human gets separate private stores per app (shared notes and entities are unaffected). A JWT deployment is what gives one human one Agno across channels.

Two implementation notes: the legacy `enable_agentic_memory` flag stays **off** on the four control-plane components that carry learning — alongside learning stores it would register the legacy MemoryManager's `update_user_memory` tool, shadowing the learning store's tool of the same name. And eval cases that probe a learning-store agent must set the learning hooks (see [Evals](#evals)), and name their fixtures things no real team would have on file. The hooks diff on row identity, so they remove rows a case *created* but cannot undo an edit *inside* a row that already existed — a superseded fact, a replaced relationship, a rewritten note line. Distinctive names are what keep a case out of that path.
